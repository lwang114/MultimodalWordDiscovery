...
ORDER='C'
class ImageAudioGMMWordDiscovererFixedVar:
  def __init__(self, speech_feature_file, image_feature_file, model_configs, model_name='image_audio_word_discoverer_fixed_var'):
    ...
    g_sb00 = model_configs.get('gamma_sb0', 0.1) # Stick-breaking beta distribution parameter
  ...
  def initialize_model(self, alignments=None, initialize_obs_model=True):
  ...
  def train_using_EM(self, num_iterations=10, write_model=True, save_alignment=True, initialize_obs_model=True):
    self.initialize_model(initialize_obs_model=initialize_obs_model)
  ...
  def train_curriculum_EM(self, syllabus_files, num_iterations=10, write_model=True):
    n_syllabus = len(syllabus_files)
    for i_s, syllabus_file in enumerate(syllabus_files):
      speech_feat_file = syllabus_file[0]
      image_feat_file = syllabus_file[1]
      self.read_corpus(speech_feat_file, image_feat_file)
      if i_s == n_syllabus - 1:
        self.train_using_EM(num_iterations, write_model=write_model, initialize_obs_model=False)
      elif i_s == 0:
        self.train_using_EM(num_iterations, write_model=False, save_alignment=False, initialize_obs_model=True) 
      else:
        self.train_using_EM(num_iterations, write_model=False, save_alignment=False, initialize_obs_model=False) 
  ...
  def compute_ELBO(self):
    ...
    kl_div_mean_a = gaussian_KL_divergence(self.audio_obs_model['means'], 
                                            ...
                                            self.fixed_variance_a / np.exp(self.k_a0) * np.tile(np.eye(self.embed_dim)[np.newaxis, np.newaxis, :, :], (self.Kmax, self.Mmax, 1, 1)), 
                                            cov_type='diag')
    kl_div_mean_v = gaussian_KL_divergence(self.image_obs_model['means'], 
                                            ...
                                            self.fixed_variance_v / np.exp(self.k_v0) * np.tile(np.eye(self.image_feat_dim)[np.newaxis, np.newaxis, :, :], (self.Kmax, self.Mmax, 1, 1)), 
                                            cov_type='diag')
...
if __name__ == '__main__':  
  ...
  if test_case == 0:
    ...
    # TODO
    # XXX
    model_configs = {'Kmax':10, 'Mmax':1, 'embedding_dim':140, 'has_null':False}  
