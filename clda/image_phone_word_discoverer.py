...
ORDER='C'
class ImagePhoneWordDiscoverer:
  def __init__(self, speech_feature_file, image_feature_file, model_configs, model_name='image_phone_word_discoverer'):
    ...
    self.Mmax = model_configs.get('Mmax', 1)
    ...
    self.g_at0 = model_configs.get('gamma_a_trans', 0.01) # Alignment transition Dirichlet priors parameter
    self.g_at0 = np.log(self.g_at0) 
    self.g_mv0 = model_configs.get('gamma_visual_mixture', 0.01)
    self.g_t0 = model_configs.get('gamma_trans_prob', 0.0001)
  ...
  def train_using_EM(self, num_iterations=10, write_model=True):
    ...
    for n in range(num_iterations):
      ...
      for ex, (afeats, vfeats) in enumerate(zip(self.a_corpus, self.v_corpus)):
        ...
        log_probs_a_given_i = self.log_probs_afeat_given_i(log_probs_a_given_z, counts_concept) 
  ...
  def update_concept_counts(self, counts_align_init, probs_afeat_given_z, probs_vfeat_given_zm):
    ...
    # XXX: An approximation, should be multiplied by the digamma of the multinomial distribution 
  ...
  def update_posterior_hyperparameters(self):
    ...
    new_g_tN = deepcopy(self.g_t0 * np.ones((self.Kmax,)))
