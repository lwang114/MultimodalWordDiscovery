...
class ImageAudioWordDiscoverer:
  def __init__(self, speech_feature_file, image_feature_file, model_configs, model_name='image_audio_word_discoverer'):
    ...
    self.embed_dim = model_configs.get('embedding_dim', 120)
  ...
  def read_corpus(self, speech_feat_file, image_feat_file, segmentations=None):
    ...
    if segmentations is not None:
      # XXX
      a_feat_corpus = [np.array(a_npz[k]) for k in sorted(a_npz, key=lambda x:int(x.split('_')[-1]))[:1]]
      self.a_corpus = [self.get_sent_embeds(a_feat, segmentation) for a_feat, segmentation in zip(a_feat_corpus, segmentations)]
    else:
      # XXX
      self.a_corpus = [np.array(a_npz[k]) for k in sorted(a_npz, key=lambda x:int(x.split('_')[-1]))[:1]]

    self.v_corpus = [np.array(v_npz[k]) for k in sorted(v_npz, key=lambda x:int(x.split('_')[-1]))[:1]] 
    ...
    self.audio_feat_dim = self.a_corpus[0].shape[-1]
  ...
  def update_audio_mixture_counts(self, probs_afeat_given_zm):
    ...
    counts_audio_mixture = probs_afeat_given_zm
  ...
  def update_image_mixture_counts(self, probs_vfeat_given_zm):
    ...
    counts_image_mixture = probs_vfeat_given_zm
    # Normalize
  ...
  def compute_ELBO(self):
    ...
    kl_div_parameters = kl_div_mean_a + kl_div_mean_v + kl_div_concept_prob
    ...
    for ex, (afeats, vfeats) in enumerate(zip(self.a_corpus, self.v_corpus)):
      ...
      kl_div_concept = KL_divergence(self.counts_concept[ex], self.concept_prior[np.newaxis, :])

      kl_div_align = KL_divergence(self.counts_align[ex][0], self.align_init[n_state])
  ...
  def align_i(self, i, decode_method='viterbi'): 
    ...
    align_best_scores = deepcopy(align_init_scores[0])
  ...
  def align(self, afeats, vfeats, decode_method='viterbi'):
    ...
    log_prob_x_given_z = logsumexp(self.audio_obs_model['weights'] + log_probs_a_given_zm, axis=-1)

    for k in range(n_states):
      concept_scores[k] = deepcopy(log_prob_z_given_v[k])
      log_prob_x_all_given_z = np.sum(align_matrix[:, k] * log_prob_x_given_z.T, axis=1) 
      concept_scores[k] += log_prob_x_all_given_z

    best_concepts = np.argmax(concept_scores, axis=1).tolist()
    return best_concepts, concept_probs, best_path[::-1], align_probs 
...
def gaussian_KL_divergence(mean1, cov1, mean2, cov2, cov_type='diag'):
  ...
  if cov_type == 'diag':
    variances1 = np.diagonal(cov1, axis1=-1, axis2=-2)
    variances2 = np.diagonal(cov2, axis1=-1, axis2=-2)
    d = mean1.shape[-1]
    tr_cov2_inv_cov1 = np.sum(variances1 / variances2)
    #print('variances1, variances2: ', variances1, variances2)
    mahalanobis = np.sum((mean2-mean1)**2 / variances2, axis=-1)
    log_det_cov1_inv_cov2 = np.sum(np.log(variances2 / variances1), axis=-1)
    return np.sum(1./2 * (tr_cov2_inv_cov1 + mahalanobis + log_det_cov1_inv_cov2 - d)) 
  if cov_type == 'standard_prior':
    variances1 = np.diagonal(cov1, axis1=-1, axis2=-2)
    #print('variances1: ', variances1)
    return 1./2 * np.sum(variances1 + mean1**2 - np.log(variances1) - 1.)      
...
if __name__ == '__main__': 
  ...
  best_path, best_concepts, align_probs, concept_probs = model.align(image_feats['0'], image_feats['0'])
  print('align_probs, concept_probs: ', align_probs, concept_probs)
  best_path, best_concepts, align_probs, concept_probs = model.align(image_feats['1'], image_feats['1'])
  print('align_probs, concept_probs: ', align_probs, concept_probs)
  best_path, best_concepts, align_probs, concept_probs = model.align(image_feats['2'], image_feats['2'])
  print('align_probs, concept_probs: ', align_probs, concept_probs)
  print('model.counts_concept: ', model.counts_concept)  
  ...
  alignments = None
  model_configs = {'Kmax':3, 'Mmax':1, 'embedding_dim':3, 'k_a0':0, 'k_v0':0, 'alignments':[alignments]}  
  ...
  model_configs = {'Kmax':100, 'Mmax':1, 'embedding_dim':120, 'k_a0':0, 'k_v0':0, 'alignments':[segment_alignments], 'segmentations':segmentations}  
