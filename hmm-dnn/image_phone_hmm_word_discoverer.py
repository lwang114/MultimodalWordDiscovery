...
class ImagePhoneHMMWordDiscoverer:
  ...
  def initializeModel(self, alignments=None):
    ...
    self.W = 0.1 * np.random.normal(size=(self.nWords, self.imageFeatDim))  
  ...
  def trainUsingEM(self, numIterations=20, writeModel=False, warmStart=False, convergenceEpsilon=0.01, printStatus=True, debug=False):
    ...
    for epoch in range(numIterations): 
      ...
      # XXX 
      normFactor = np.maximum(np.sum(phoneCounts, axis=-1), EPS) 
  ...
  def updateInitialCounts(self, forwardProbs, backwardProbs, vSen, aSen, debug=False):
    ...
    for t in range(T):
      # XXX
      initExpCounts += np.sum(forwardProbs[t] * backwardProbs[t], axis=-1) / np.maximum(np.sum(forwardProbs[t] * backwardProbs[t]), EPS)
      if debug:
  ...
  def updateTransitionCounts(self, forwardProbs, backwardProbs, vSen, aSen, debug=False):
    ...
    for t in range(T-1):
      ...
      # XXX
      transExpCount /= np.maximum(np.sum(transExpCount), EPS)
  ...
  def updateSoftmaxWeight(self, conceptCounts, debug=False):
    ...
    for vSen, conceptCount in zip(self.vCorpus, conceptCounts):
      z_prob = self.softmaxLayer(vSen, debug=debug) 
...
if __name__ == '__main__':
  ...
  if 0 in tasks:
    ...
    modelConfigs = {'has_null': False, 'n_words': 3, 'learning_rate': 10.}
    ...
    model.printAlignment('exp/dec_11_tiny/tiny', debug=False)
  #-------------------------------#
  ...
  if 2 in tasks:      
    ...
    modelConfigs = {'has_null': False, 'n_words': 65, 'momentum': 0.3, 'learning_rate': 10., 'normalize_vfeat': False, 'step_scale': 1.}
    modelName = 'exp/jan_4_mscoco_momentum%.1f_lr%.2f_stepscale%d/image_phone' % (modelConfigs['momentum'], modelConfigs['learning_rate'], modelConfigs['step_scale']) 
    print(modelName)
