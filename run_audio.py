...
if start < 2 and end >= 2:
  ...
  if args.nmt:
    ...
    if args.smt_model == "gmm":
      ...
      model = SegEmbedGMMWordDiscoverer(args.num_mixtures, args.frame_dim, src_file, trg_file,
                                      embedDim=args.embed_dim,
                                      modelDir = model_dir,
                                      minWordLen=args.min_word_len, maxWordLen=args.max_word_len,
      ...
      if os.path.isfile(args.model_dir+"model_final*"): 
        model_dir = args.model_dir
      model = SegEmbedHMMWordDiscoverer(args.num_mixtures, args.frame_dim, args.embed_dim, 
                                      src_file, trg_file,
                                      modelDir = model_dir,
                                      minWordLen=args.min_word_len, maxWordLen=args.max_word_len,
                                      landmarkFile=args.preseg_file)
    ...
    if os.path.isfile(args.model_dir+'model_final*'):
      ...
      model.trainUsingEM(numIterations=args.num_iterations, writeModel=True, modelPrefix=args.model_dir)
