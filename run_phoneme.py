...
from hmm.hmm_word_discoverer import *
import argparse
import shutil
import time
from utils.postprocess import * 
...
parser.add_argument("--smt_model", choices={"simplified", "enriched", "segmental"}, help="Type of SMT model")
...
if start < 2 and end >= 2:
  ...
  elif args.smt_model == "simplified":
    model = IBMModel1(datapath + 'phoneme_level/flickr30k.txt', modelName=args.exp_dir + args.smt_model)
    model.initializeWordTranslationProbabilities(smt_model_path)
  elif args.smt_model == "enriched":
    model = IBMModel2(datapath + 'phoneme_level/flickr30k.txt')
  elif args.smt_model == "segmental":
    if os.path.isfile(args.exp_dir+"model_final*"):
      model = HMMWordDiscoverer(datapath + "phoneme_level/flickr30k.txt",
                              args.exp_dir + "model_final_initialprobs.txt",
                              args.exp_dir + "model_final_transitionprobs.txt",
                              args.exp_dir + "model_final_observationprobs",
                              modelName=args.exp_dir + "hmm_word_discoverer")
    else:
      model = HMMWordDiscoverer(datapath + "phoneme_level/flickr30k.txt",
...
if start < 4 and end >= 4:
  ...
  if args.nmt:
    ...
    gold_alignment_file = gold_alignment_nmt_file
  else:
    pred_alignment_file = pred_alignment_smt_prefix+".json" 
    gold_alignment_file = gold_alignment_smt_file
